...
- **Physical Tello (djitellopy, no ROS):** **`tello_real_autonomy_v1.py`** — preview → takeoff → **SEARCH** (**SPIN** cold vs recovery yaw, **SETTLE** + **ACQ** at zero yaw) → **FACE_LOCK** (**PD**); **`--gesture-zone-top-frac`**; **`--search-yaw-rc`** / **`--search-yaw-rc-recovery`** — STATUS **SEARCH / FACE_LOCK policy**. **thumbs_up** / **two_fingers** / **open_palm** + worker queue; **`--test-yaw`**; **`tello-autonomy`**. Land **Q** / Ctrl+C. **`mlx_djitellopy_udp_video`** via **`tello_view`**. **`tello_hover_baseline.py`** / **`tello_real_flight_test.py`** as before.
...
*On the **real drone**, **`tello_real_autonomy_v1.py`** uses **fist** (→ SEARCH), **thumbs_up** (→ queued **`move_up`**), **two_fingers** (→ queued **`move_forward`**, **`MOVE_FORWARD_CM`** default **50 cm**, clamped to SDK **20–500 cm**), **open_palm** (→ queued **land**). Bridge/sim still use **`two_fingers` → FOLLOW_ARM** (preview / ROS); physical autonomy maps peace sign to **forward SDK motion**, not FOLLOW_ARM.*
...
3. **STATUS.md** -- What works now, what's broken, session-log diagnostics, dataset stats, prioritized next steps. **This file is the living document -- update it after completing any work.** For gesture timing and **`tello_real_autonomy_v1`** control flow, read **Gesture Confirmation System** and **Physical autonomy v1 — control & threading** in that file.
...
7. **Gesture module split:** **`gesture_filter.py`** = **`GestureFilter`** only (bridge-safe). **`simulate_drone.py`** still holds classifier session helpers **`tello_real_autonomy_v1`** reuses — see STATUS/ARCHITECTURE if moving imports.
