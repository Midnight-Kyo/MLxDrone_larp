...
| `.gitignore` | venv, datasets, `*.pt`, `hagrid_detection/`, logs, `**/runs/`, … |
...
Same as before: `gesture_model.pt`, `yolo_hands/weights/best.pt`, datasets under `gesture_drone/` (see STATUS for paths; large trees gitignored).
...
- [ ] **Windows:** venv active (or `MLX_PYTHON` set), `pip install -r requirements.txt`, project weights present or downloadable (e.g. Hugging Face fallback for YOLO).
