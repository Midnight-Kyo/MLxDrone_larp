...
- **`tello_real_autonomy_v1.py`** — pre-flight OpenCV preview → optional climb prompt → takeoff → settle (zero RC) → **SEARCH** → **FACE_LOCK** (**PD**: **`rate_rad = KP_LOCK_FACE * ex + KD_LOCK_FACE * d_error`** — **positive `yaw_rc`** toward face on right). **SEARCH (`rc`):** **SPIN** with **`--search-yaw-rc`** (default **±40**, cold) or **`--search-yaw-rc-recovery`** (default **±28**, directional after face loss); **`--search-mode cw`** for discrete rotates. While **`face_ok`**: **SETTLE** **`T_FACE_SETTLE_S` = 0.2** s then **ACQ** **`T_FACE_ACQUIRE_S` = 0.2** s at **`yaw_rc = 0`**, then lock (see STATUS). **First SEARCH frame** with **`face_ok`**: **SETTLE** start, not immediate acquire. **Vertical gesture zone:** **`--gesture-zone-top-frac`** (default **0.6**); HUD **line** + **gray** bbox (**`draw_cam_panel`**). **Loss** **`T_FACE_LOSS_S`**. **`AUTONOMY_YUNET_FRAME_STRIDE` = 2**; orange face HUD; **`djitellopy`** **WARNING** before RC worker; **`--test-yaw`**. **`tello-autonomy`** (**`python -u`**). **Land:** **open_palm** / **Q** / Ctrl+C. **`MOVE_FORWARD_CM` = 50**. **`SafeTello`**; **`tgate = None`**; **~25 Hz** worker — STATUS **Physical autonomy v1**.
...
| `simulate_drone.py` | Windows | 2D sim + `hand_detection` + SessionLogger + scale HUD + `--source` / `--world-width-m`; optional **SEARCH / FACE_LOCK** on heading; **`draw_cam_panel`** supports optional gesture-zone line + hand bbox color (used by **`tello_real_autonomy_v1`**). |
...
| `tello_real_autonomy_v1.py` | Windows | **Physical** djitellopy: preview → climb → takeoff → SEARCH (**SPIN/SETTLE/ACQ**, cold vs **`--search-yaw-rc-recovery`**) → FACE_LOCK (**PD**); **`--gesture-zone-top-frac`**; **`--test-yaw`**; no ROS; **`gesture_filter` + `simulate_drone`**. |
| `tello_autonomy_launcher.py` | Windows | **`tello-autonomy`** entry: print banner then **`import tello_real_autonomy_v1`**; **`python -u`**. |
...
| `search_behavior.py` | Windows / WSL | Shared **`face_ok_and_x_norm`**, **`EPS_X`**, **`MAX_ANG_LOCK`**, **`M_ACQUIRE`**, **`M_LOSS`**, **`OMEGA_SEARCH`**, **`KP_LOCK`** (P-only on **sim / ROS / bridge**). **Physical** **`tello_real_autonomy_v1`** adds **local** **`KP_LOCK_FACE` / `KD_LOCK_FACE`** PD for FACE_LOCK only — does **not** change this file. |
...
**`Load-PowerShellAliases.ps1`** defines **`drone`**, **`bridge`**, **`simulate`**, **`tello`** (HUD), **`tello-realtest`**, **`tello-autonomy`** (runs **`tello_autonomy_launcher.py`** with **`-u`**), and **`mlx-help`**. Users often duplicate **`$PROFILE`** entries or extend with wrappers (e.g. **`simulate-tello`** → `simulate_drone.py --source tello`, **`tello-hover`** → `tello_hover_baseline.py`) using the same **`$PSScriptRoot`** pattern.
...
- **`tello_real_autonomy_v1.py`:** After connect/stream, **pre-takeoff OpenCV preview** (full HUD). **[T]** arms “ready to fly”; **console** climb cm + **Enter** starts **takeoff**; **[Q]** in the preview window aborts **before** takeoff (**Ctrl+C** aborts preview the same way). **Gestures** count only when the hand bbox **center** is in the **top** **`--gesture-zone-top-frac`** of the frame (default **0.6**; **`1.0`** = off); see STATUS for HUD line / gray bbox. Post-takeoff: short settle (**zero RC**), then **SEARCH** — **SPIN** (**cold** vs **recovery** RC magnitudes; **`cw`** optional), then **SETTLE** + **ACQ** (zero yaw, **0.2 s** each) before **FACE_LOCK** (**PD** — STATUS **SEARCH / FACE_LOCK policy**). **Climb:** confirmed **thumbs_up** enqueues **`("MOVE_UP", cm, False)`**; optional **post-takeoff** climb uses **`(..., True)`** and an explicit **`rc 0 0 0 0`** + longer pre-**`move_up`** delay (see STATUS). **Forward:** confirmed **two_fingers** enqueues **`("MOVE_FORWARD", MOVE_FORWARD_CM)`** (**SDK `move_forward`**, default **50 cm**, clamped **≥20 cm**). **`move_pending`** closes the enqueue race with **`command_busy`** for both discretionary motions. **Land:** GestureFilter-confirmed **open_palm** (queued **`land`**), or **[Q]** / Ctrl+C; **`finally`** zeros yaw RC, **`streamoff`**, **`end()`**, destroys windows. **Instance:** **`SafeTello()`** (safe teardown). **Perception:** **`perception["tgate"] = None`** — TrustedHand off; **`GestureFilter`** from **`gesture_filter.py`**; classifier/HUD helpers still from **`simulate_drone`**. **Threading:** main loop does perception and pushes **yaw RC** + **`command_q`**; a **~25 Hz worker** runs **`move_up`** / **`move_forward`** / **`land`**, **`send_rc_control`** only when not busy and **yaw ≠ 0**, **keepalive** **`rc 0 0 0 0`** when yaw stays **0** for **~10 s** (see STATUS for skip / SDK **`move_*`** timing), and reports **`MOVE_UP_DONE`** / **`MOVE_FORWARD_DONE`** / **`LAND_DONE`** / **`MOTOR_STOP`** (details: STATUS **Physical autonomy v1 — control & threading**). **Logging:** takeoff-relative **`[+t]`** on **`_flight_print`**; SEARCH phase / settle / acq lines per STATUS; **djitellopy** library log level **WARNING** to hide per-**`rc`** INFO.
